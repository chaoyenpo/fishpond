...
namespace Gamesmkt\Fishpond\BetRecord;
...
class BetRecordParser
{
    ...
    public function execute(): array
    {
        ...
        foreach ($this->records as $record) {
            // TODO: 確認此功能是否有需要
            // 因為這裡的注單是從這間廠商拉取的，如果不是這個平台的玩家產生的注單就要忽略。
            // if (!$this->format->getPlayer($betRecord)) {
            //     continue;
            // }
    ...
    public function toModel($sourceRecord): Record
    {
        ...
        // 派彩金額
        $record->payment = $record->order_status === BetRecordInterface::STATUS_ACTIVE
        ? ''
    ...
    {
        if ($this->format->getStatusCode($betRecord) === BetRecordInterface::STATUS_ACTIVE) {
            return '';
        }
